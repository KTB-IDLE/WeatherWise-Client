  ...
  return (
    <div className="profile-page">
      ...
      <div className="profile-header">
        ...
        <div className="profile-info">
          <div className="nickname-level">
           <span className="nickname">{userInfo.nickname || '닉네임 없음'}</span> {/* 1. 닉네임 표시 */}
            <span className="level">LEVEL 12 <span className="rank">(랭킹 41%)</span></span>
          </div>
      ...
      <div className="profile-section">
        ...
        <ul>
          <li className="account-item">
            <span>아이디</span>
            <span className="email">{userInfo.serialId || '이메일 없음'}</span> {/* 2. 이메일(Serial ID) 표시 */}
          </li>
          <li onClick={() => { setShowPopup(true); setPopupContent('닉네임을 입력하세요:'); }}>닉네임 변경</li>
          <li onClick={() => { setShowPopup(true); setPopupContent('로그아웃 하시겠습니까?'); }}>로그아웃</li>
          <li onClick={() => { setShowPopup(true); setPopupContent('정말 탈퇴하시겠습니까?'); }}>회원 탈퇴</li>
        </ul>
      </div>
      ...
      <div className="profile-section">
        <h2>커뮤니티</h2>
        <ul>
          <li onClick={() => window.location.href = '/myposts'}>내가 작성한 글</li>
        </ul>
      </div>
